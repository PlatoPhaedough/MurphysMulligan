...
public class TerrainGenerator : MonoBehaviour 
{
	...
	public float m_splatTileSize0 = 5.0f;
	...
	public int m_detailObjectDistance = 2000; //The distance at which details will no longer be drawn
	...
	void Start() 
	{
		...
		m_mountainSeed = (int) temp;
		float temp2 = Random.Range (0.0f, 50.0f);
		m_groundSeed = (int) temp2;
		float temp4 = Random.Range (0.0f, 50.0f);
		m_detailSeed = (int) temp4;
		float temp6 = Random.Range (0.0f, 50.0f);
		m_treeSeed = (int) temp6;*/
		...
		StartCoroutine(CreateProtoTypes());
		...
		for(int x = 0; x < m_tilesX; x++)
		{
			for(int z = 0; z < m_tilesZ; z++)
			{
				...
				float temp5 = Random.Range (-3550.0f, 3550.0f);
				m_mountainSeed = (int) temp5;
				StartCoroutine(FillHeights(htmap, x, z));
				terrainData = new TerrainData();
				//Debug.Log ("Middle1 " + System.DateTime.Now.ToString());
				...
				StartCoroutine(FillAlphaMap(terrainData));
				//Debug.Log ("Middle2 " + System.DateTime.Now.ToString());
				m_terrain[x,z] = Terrain.CreateTerrainGameObject(terrainData).GetComponent<Terrain>();
				GameObject terrain = GameObject.FindGameObjectWithTag("TerrainGenerator");
				//m_terrain[x,z].transform.position = new Vector3(terrain, 0, m_terrainSize*z + m_offset.y);
				...
				StartCoroutine(FillTreeInstances(m_terrain[x,z], x, z));
				StartCoroutine(FillDetailMap(m_terrain[x,z], x, z));
		...
		for (int i = 0; i < 1; i++) {
			GameObject test = GameObject.Find ("Terrain");
			if (test != null)
				test.AddComponent<Rigidbody> ();

			test.rigidbody.useGravity = false;
			test.rigidbody.isKinematic = true;
			test.tag = "MainTerrain";
		}

		NodeGeneratorController ngc = GameObject.FindGameObjectWithTag ("NodeGenerator")
			.GetComponent<NodeGeneratorController> ();

		ngc.begin = true;
	}
	
	IEnumerator CreateProtoTypes()
	{
	...
	public float GetSteepnessValue(Vector3 node)
	{
		Vector3 temp = new Vector3 ();
		temp.y = Terrain.activeTerrain.SampleHeight (node);
		//Debug.Log (temp.y);
		
			return temp.y;
	}
		
	IEnumerator FillHeights(float[,] htmap, int tileX, int tileZ)
	{
	...
	IEnumerator FillAlphaMap(TerrainData terrainData) 
	{
	...
	IEnumerator FillTreeInstances(Terrain terrain, int tileX, int tileZ)
	{
		Random.seed = 0;
	...
	IEnumerator FillDetailMap(Terrain terrain, int tileX, int tileZ)
	{
		...
		for(int x = 0; x < m_detailMapSize; x++) 
		{
            for (int z = 0; z < m_detailMapSize; z++) 
			{
                ...
                float normX = x * unit;
                float normZ = z * unit;
				//Debug.Log("normx =" + normX + " normz =" + normZ);
                // Get the steepness value at the normalized coordinate.
                float angle = terrain.terrainData.GetSteepness(normX, normZ);
