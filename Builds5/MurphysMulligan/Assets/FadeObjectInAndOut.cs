...
public class FadeObjectInAndOut : MonoBehaviour
{
	...
	private bool startIn = false, startOut = false;
	private int idx = 0;
	public bool hole_selected = false, reset = false;
	...
	void Update()
	{
		if (gameObject.activeSelf == true && idx == 0 && gameObject.name != "miniMapHole") {
			StartCoroutine("FadeSequence", -2);
			idx++;
		} else if(gameObject.name == "miniMapHole") {
			Node grab_script = GameObject.FindGameObjectWithTag("Hole").GetComponent<Node>();
			if(grab_script.isSelected && reset)
			{
				StartCoroutine ("FadeSequence", -2);
				reset = false;
			}
